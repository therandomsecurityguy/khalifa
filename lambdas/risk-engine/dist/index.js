...
exports.resolveStaleIssuesExample = resolveStaleIssuesExample;
exports.demonstratePeriodicRunner = demonstratePeriodicRunner;
...
async function runSingleRuleExample() {
    ...
    const runner = new runner_1.RiskRuleRunner(NEPTUNE_ENDPOINT);
    try {
...
async function computeRiskScoreExample() {
    ...
    for (let i = 0; i < testInputs.length; i++) {
        const result = (0, scoring_1.computeRiskScore)(testInputs[i]);
...
async function resolveStaleIssuesExample() {
    ...
    try {
        const resolvedCount = await (0, runner_1.resolveStaleIssues)(NEPTUNE_ENDPOINT);
...
async function main() {
    ...
    console.log(`  Available Rules: ${rules_1.riskRules.length}`);
    console.log(`\n`);
